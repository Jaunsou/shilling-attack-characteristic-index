...
for i in range(0,943):
    ...
    for j in range(0,1682):
        if df1.iloc[i,j] == 0:
            continue
        w = (math.fabs(df1.iloc[i,j] - Avg_rating[j])) / (Count[j])**2
        W = W + w
    WDA_Ij.append(W)
...
for index,row in df1.iterrows():
    ...
    rating_count = row_count[1:]
    count = rating_count[1] + rating_count[2] + rating_count[3] + rating_count[4] + rating_count[5]
    WDMA = WDA_Ij[index-1] / count
    print('用户',index,'加权的评分平均偏移度',WDMA)
    df2.iloc[index - 1, 0] = WDMA
# print(df2)
# df2.to_csv(r'c:\Users\20795\PycharmProjects\movielens\\WDMA.csv')